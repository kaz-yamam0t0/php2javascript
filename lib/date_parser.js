...
const scan = (data, s, pos)=>{
	...
	// iso interval
		...
		if (_y != 0 && !isNaN(_y)) data.additions.push({n:_y, unit:"year"});
		if (_m != 0 && !isNaN(_m)) data.additions.push({n:_m, unit:"month"});
		if (_d != 0 && !isNaN(_d)) data.additions.push({n:_d, unit:"day"});
		if (_h != 0 && !isNaN(_h)) data.additions.push({n:_h, unit:"hour"});
		if (_i != 0 && !isNaN(_i)) data.additions.push({n:_i, unit:"minute"});
		if (_s != 0 && !isNaN(_s)) data.additions.push({n:_s, unit:"sec"});
