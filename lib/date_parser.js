...
const scan = (data, s, pos, add_c=1)=>{
	...
	else if (g = s_.match(/^([\+\-]?)\s*(\d+|a)\s*(year|month|day|hour|minute|second|week|millisecond|microsecond|msec|ms|µsec|µs|usec|sec|min|forth?night)s?(\s+ago)?\b/i)) {
		let [sign, n, unit, ago] = [g[1],g[2],g[3].toLowerCase(), g[4]];
		if (n == "a") n = 1;
		
		n = +n;
		if (sign === "-") n *= -1;
		if (ago) n *= -1;
		if ( !(ago && add_c != -1) ) { // ago is always ago
			n *= add_c; 
		}
	...
	// iso interval
	else if (g = s_.match(/^P((\d+)Y)?((\d+)M)?((\d+)D)?(T((\d+)H)?((\d+)M)?((\d+(\.\d+)?)S)?)?\b/i)) {
		let [_y, _m, _d, _h, _i, _s] = [+g[2],+g[4],+g[6],+g[9],+g[11],+g[13]]; // +undefined -> NaN
		if (_y != 0 && !isNaN(_y)) data.additions.push({n:_y * add_c, unit:"year"});
		if (_m != 0 && !isNaN(_m)) data.additions.push({n:_m * add_c, unit:"month"});
		if (_d != 0 && !isNaN(_d)) data.additions.push({n:_d * add_c, unit:"day"});
		if (_h != 0 && !isNaN(_h)) data.additions.push({n:_h * add_c, unit:"hour"});
		if (_i != 0 && !isNaN(_i)) data.additions.push({n:_i * add_c, unit:"minute"});
		if (_s != 0 && !isNaN(_s)) data.additions.push({n:_s * add_c, unit:"sec"});
...
// if subtracting intervals, `add_c` = -1
const parse = (datetime, t=null, add_c=1)=>{
	...
	while(cnts++ < 15 && pos < datetime.length) {
		if ((pos = scan(data, datetime, pos, add_c)) < 0) {
